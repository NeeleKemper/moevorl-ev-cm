...
class MOPolicy(ABC):
    ...
    def report(
        ...
        wandb.log(
            {
                # f"{log_name}{idstr}/scalarized_return": scalarized_return,
                # f"{log_name}{idstr}/scalarized_discounted_return": scalarized_discounted_return,
                f"{log_name}{idstr}/success_rate": 1 - constraint_violations,
        ...
        wandb.log(
            {
                f"{log_name}{idstr}/scalarized_return": scalarized_return,
                f"{log_name}{idstr}/scalarized_discounted_return": scalarized_discounted_return,
                f"{log_name}{idstr}/constraint_violations": constraint_violations,
                "global_step": self.global_step,
            }
        )
        for i in range(vec_return.shape[0]):
    ...
    def policy_eval_evorl(self,
        ...
        (
            ...
            discounted_vec_return,
            constraint_violations,
        ) = policy_evaluation_evorl(self, eval_env, w=weights, rep=num_episodes)
    ...
    def policy_eval(
        ...
        return scalarized_return, scalarized_discounted_return, vec_return, discounted_vec_return, constraint_violations
...
class MOAgent(ABC):
    ...
    def setup_wandb(self, project_name: str, experiment_name: str, entity: Optional[str] = None) -> None:
        ...
        self.full_experiment_name = f"{experiment_name}_{int(time.time())}"
        import wandb
